...
| API Key UI — Gate 1 (Iteration → Documentation) | 2026-04-05 | Project Lead | Approved | ARD and PRD approved. Settings section (not separate page), useCanManageApiKeys hook, Apollo Client for existing GQL ops, copy-to-clipboard on create, revoke with confirmation. |
| API Key UI — Gate 2 (Documentation → Implementation) | 2026-04-05 | Project Lead | Approved | Technical Spec, Deployment Plan, Changelog complete. 3 new files, 3 modified files, web-only change, no API changes needed. |
